...
def getMyFont():
    ...
    # font0.set_file(os.path.join(selfpath, 'OpenSans-Regular.ttf'))
    font0.set_file(os.path.join(selfpath, 'simhei.ttf'))
    # font0.set_file(os.path.join(selfpath, 'Dense-Regular.otf'))
    return font0
...
def draw_RSD_Bar(filename, x, deviationValue, titledict, tl_list, tr_list, ab=None):
    ...
    if ab is not None:
        ...
        ax.set_ylim((-2., 0.5))
    add_annotate(ax, tl_list, 'left', 'r')
...
def add_title(titledict):
    ...
    添加大标题和xy轴名称
    '''
    tt = plt.title(titledict['title'], fontsize=12, fontproperties=font0)
...
def add_annotate(ax, strlist, loc, color):
    '''
    添加上方注释文字
...
def draw_Scatter(x, y, filename, titledict, tl_list, tr_list):
    ...
    fig.subplots_adjust(top=0.95, bottom=0.08, left=0.07, right=0.89)
    ...
    add_annotate(ax, tl_list, 'left', 'r')  # 注释文字
    add_annotate(ax, tr_list, 'right', '#9300d3')
    add_title(titledict)
    set_tick_font(ax)
...
def draw_map_with_colormap_interp(lons, lats, values, box, filename, title,
    ...
    result = mpl_toolkits.basemap.interp(values, lon_new, lat_new, \
